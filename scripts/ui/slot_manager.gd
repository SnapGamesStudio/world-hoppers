...
func _process(delta: float) -> void:
	...
	## test
		...
		#var closest_slot = get_closest_slot(get_global_mouse_position(),["hotbar","inventory","blueprint"])
		#print(closest_slot)
...
func temp_visuals_update(delta:float):
	...
	if current_selected_slot != null:
		...
		if current_selected_slot.item is Blueprint:
			if current_selected_slot.item.unique_name == "blueprint_station_craftable":
				closest_slot = get_closest_slot(get_global_mouse_position(),["blueprint"]) ## dont want someone to remove the station from the player
			else:
				closest_slot = get_closest_slot(get_global_mouse_position(),["hotbar","inventory","blueprint"]) # normal
				
			Globals.blueprint_hovered.emit(current_selected_slot.item,current_selected_slot)
		else:
